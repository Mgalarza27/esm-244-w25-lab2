...
```
- dropping na data points and renaming titles 
...
summary(mdl3)
```
adjusted R-squard is 0.97.. (depending on model)
...
AIC(mdl,mdl2,mdl3)
```
*Lowest AIC is the most likely fit model
In this case, it is model 2. 
...
- split data into test and training using random sampling 
- what metric to measure which model is best fit (this example using Root Mean Squared Error)
- for loop: apply rhe model to each training set
-make prediction on test set with fitted training model 
- close loop 
- summarize our RMSE (which model is best)
- final model with full data set 
...
sample(ex,size=10)
#above doesn't work because there is no replacement 
